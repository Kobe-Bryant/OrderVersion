...
-(void)accessService{
	...
	NSMutableDictionary *jsontestDic = [NSDictionary dictionaryWithObjectsAndKeys:[Common getVersion:ABOUTUS_ID],@"body-ver",[Common getVersion:BRANCH_ID],@"branch-ver",[NSNumber numberWithInt: shop_id],@"shop-id",[NSNumber numberWithInt: site_id],@"site-id",nil];
...
-(void)update{
	...
	if (gradeArray != nil && [gradeArray count] > 0) {
		...
		if (updateGradeUrl != nil && [updateGradeUrl length] > 0) {
			...
			mo6.Title = [morelistDictionary objectForKey:goGrade_KEY]; // dufu mod 2013.06.17
			mo6.link = nil;
			mo6.content = @"去给定制版打个分，评价一把";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([mo.Title isEqualToString:[morelistDictionary objectForKey:abountWe_KEY]]) {
        ...
        img = [[UIImage alloc]initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"去给定制版评个分" ofType:@"png"]];
    } else {
...
//									 leftbtn:@"稍后再说 " rightbtn:@"立即更新" url:updateUrl onViewController:self.navigationController];
            UpdateAppAlert *alert = [[UpdateAppAlert alloc]
									 initWithContent:@"检测到新版本" content:[array objectAtIndex:versioninfo_remark]
									 leftbtn:@"稍后提示我" rightbtn:@"立即更新" url:updateUrl onViewController:self.navigationController];
