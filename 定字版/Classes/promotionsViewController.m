...
- (void)viewDidLoad {
    ...
    self.tableView.frame = CGRectMake(0, 0, 320, VIEW_HEIGHT - 20.0f - 44.0f - 49.0f);
...
-(void)accessService{
	...
	CommandOperation *commandOpertmp = [[CommandOperation alloc]initWithReqStr:reqStr command:PROMOTIONS delegate:self params:nil];
...
-(void)update{
	...
	self.promotionList = [DBOperate queryData:T_PROMOTIONS theColumn:nil theColumnValue:nil withAll:YES];
	if (self.progressHUD) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	browser.newsTitle = [promotion objectAtIndex:promotions_title];
	browser.newsDesc = [promotion objectAtIndex:promotions_desc];
	
	
	browser.newsID = promotionId;
	browser.linkurl = [promotion objectAtIndex:promotions_url];
	browser.linktitle = [promotion objectAtIndex:promotions_title];
	browser.moduleType = [promotion objectAtIndex:hot_type];
	browser.commentCount = [[promotion objectAtIndex:promotions_comment_counts] intValue];
...
-(UIImage*)getPhoto:(NSIndexPath *)indexPath{
	NSLog(@"getPhoto");
	 ...
	 NSString *picName = [one objectAtIndex:promotions_pic_name];
	 if (picName.length > 1) {
...
-(NSString*)getPhotoURL:(NSIndexPath *)indexPath{
	NSArray *pro = [promotionList objectAtIndex:[indexPath row]];
	return [pro objectAtIndex:promotions_pic];
}
-(bool)savePhoto:(UIImage*)photo atIndexPath:(NSIndexPath*)indexPath{
	...
	NSString *photoname = [callSystemApp getCurrentTime];
	 ...
	 {
	 NSArray *one = [promotionList objectAtIndex:[indexPath row]]; 
	 NSNumber *value = [one objectAtIndex:promotions_id];
	 [FileManager removeFile:[one objectAtIndex:promotions_pic_name]];
	 [DBOperate updateData:T_PROMOTIONS tableColumn:@"pic_name" columnValue:photoname conditionColumn:@"id" conditionColumnValue:value];
	 }
}
