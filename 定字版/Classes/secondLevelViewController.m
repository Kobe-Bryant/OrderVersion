...
-(void)showTypeBto:(NSMutableArray*)titleArray{
    ...
    NSUInteger pageNum = [titleArray count]/3;
    ...
    for (int i = 0; i < [titleArray count]; i++) {
        ...
        btitle.textAlignment = NSTextAlignmentCenter;
        btitle.tag = 99;
...
- (void)startIconDownload:(NSString*)imageURL forIndex:(NSIndexPath*)index
{
	...
	[index getIndexes:&i];
	myImageView *myIV = (myImageView *)[scrollView viewWithTag:(i + 100)];
	NSLog(@"startIconDownload index ---------  @%lu",(unsigned long)i);
	[myIV startSpinner];
...
- (void)appImageDidLoad:(NSIndexPath *)indexPath withImageType:(int)Type
{
    ...
    myImageView *myIV = [scrollView viewWithTag:(index + 100)];
    NSLog(@"appImageDidLod index ---------  @%lu",(unsigned long)index);
    [myIV stopSpinner];
...
//                NSLog(@"one %d ",[one count]);
        ...
        if ([imageDownloadsInWaiting count]>0) {
            ...
            myImageView *myIV = [scrollView viewWithTag:(i + 100)];
            [myIV stopSpinner];
