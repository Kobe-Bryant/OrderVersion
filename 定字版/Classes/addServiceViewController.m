...
- (void)viewDidLoad {
	...
	for (int tag = FIRST_TEXTFIELD_TAG; tag <= LAST_TEXTFIELD_TAG; tag++) {
		UITextField *tempTextField = (UITextField *)[self.view viewWithTag:tag];
		if (addinfo != nil) {
			isEdit = YES;
			switch (tag) {
				case 101:
	...
	UIButton *showMapBto = (UIButton *)[self.view viewWithTag:SHOWMAP_TAG];
...
//		UITextField *one = (UITextField *)[self.view viewWithTag:i];
...
-(IBAction)textFieldDone:(id)sender {
	...
	if (tempTextField.tag == LAST_TEXTFIELD_TAG) {
		tempTextField = (UITextField *)[self.view viewWithTag:FIRST_TEXTFIELD_TAG];
	}
	else {
		tempTextField = (UITextField *)[self.view viewWithTag:(tempTextField.tag+1)];
	}
...
{
	UITextField *textField = (UITextField *)[self.view viewWithTag:currentTextFieldTag];
	[textField resignFirstResponder];
	NSTimeInterval animationDuration = 0.30f;
	[UIView beginAnimations:@"ResizeForKeyboard" context:nil];
	[UIView setAnimationDuration:animationDuration];
