...
- (void)showToolBar{
	...
	UIToolbar *myToolBar = [[UIToolbar alloc] initWithFrame:  
							CGRectMake(0.0f, [UIScreen mainScreen].bounds.size.height - 20 - 44, 320.0f, 44.0f)];
...
//	[commentBtn setFrame:CGRectMake(offset, 4, 24, 24)];
	...
	[commentBtn setImage:[UIImage imageNamed:@"下载icon.png"] forState:UIControlStateNormal];
	[commentBtn addTarget:self action:@selector(HandleLoadDown:) forControlEvents:UIControlEventTouchUpInside];
	
	UILabel *commentlabel = [[UILabel alloc] initWithFrame:CGRectMake(offset, 28, 30, 14)];
	commentlabel.text = @"下载";
...
//					wbengine.expireTime = expiresTime;
					...
					NSString *param = [NSString stringWithString:SHARE_CONTENT];
					NSString *param1 = [NSString stringWithFormat:@"[壁纸%@]  %@",content,linkurl];
					param1 = [param stringByAppendingString:param1];
		...
		switch (index) {
			case 0:
			{
				...
				if (app_wechat_share_type == app_to_wechat) {
					[sendMsg sendImageContent:[img fillSize:CGSizeMake(114, 114)]];
				}else if (app_wechat_share_type == wechat_to_app) {
					[sendMsg RespImageContent:[img fillSize:CGSizeMake(114, 114)]];
				}	
				[sendMsg release];	
				break;
			}				
...
-(IBAction)HandleToWeChat:(id)sender{
	if(![WXApi isWXAppInstalled]){
		...
		[actionsheet1 release];
		[weChatActionSheet showActionSheet:self.view];		
	}
}
